...
function rowHTML(e, cfg) {
  ...
  return `<tr class="${e.status === 'cancelado' ? 'row-muted' : ''}">
    <td data-label="Data">${fmtData(e.data)}</td>
    <td data-label="Dia"><span class="badge badge-plum">${diaSemana(e.data)}</span></td>
    <td data-label="Horário" class="fw-600" style="color:var(--plum)">${e.horario || '—'}</td>
    <td data-label="Cliente" class="fw-600">${e.cliente || '—'}</td>
    <td data-label="Serviço">${e.servicoNome || '—'}</td>
    <td data-label="Profissional"><span class="badge badge-rose">${e.profissional || '—'}</span></td>
    ...
    <td data-label="WhatsApp" class="td-actions">
      ${temTel ? `
